...
When Codex finishes the work for which it showed the view, it calls `hide_anki_review` with that view's ID. The widget checks its own visibility state, clears card content, and asks the host to close it. It also requests a minimal one-pixel height if the host keeps the iframe. The host owns the chat tool-result container and may leave its header visible. Hiding does not grade a card, delete the review session, or discard a pending rating. The repository's `AGENTS.md` instructs Codex to send this hide signal before each final answer after opening a view.
