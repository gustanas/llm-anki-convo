...
export async function normalizeAnkiCards(rawCards, {
  ...
  for (const raw of rawCards) {
    ...
    // Anki replaces sound tags with internal player IDs in rendered HTML. A
    // single conventional Audio/Sound field can safely resolve a single player.
    for (const side of ['question', 'answer']) {
      let html = sides[side];
      const players = [...html.matchAll(/\[anki:play:[qa]:\d+\]/g)];
      if (players.length) {
        const sound = raw.fields?.Audio?.value || raw.fields?.Sound?.value || '';
        const refs = mediaReferences(sound).filter((ref) => ref.type === 'audio');
        if (players.length === 1 && refs.length === 1) html += sound;
        else warnings.add('Some custom Anki audio players could not be resolved; review those cards in Anki.');
      }
      // Audio first, so large illustrations cannot consume the listening budget.
      const refs = mediaReferences(html).sort((a, b) => (a.type === 'audio' ? 0 : 1) - (b.type === 'audio' ? 0 : 1));
      for (const ref of refs) {
        let filename = ref.filename;
        try { filename = decodeURIComponent(filename); } catch { /* Literal filenames may include %. */ }
